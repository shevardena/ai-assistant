...
class CreateSupportTicketTool implements BotTool
{
    private const OPERATION_IDENTIFIER = 'create_support_ticket';
    ...
    {
        return 'Create a support ticket through the configured support integration after explicit confirmation.';
    }
    ...
    public function execute(Bot $bot, array $arguments, ToolExecutionContext $context): ToolResult
    {
        ...
        if ($inputs === null) {
            return ToolResult::failure(
                'invalid_arguments',
                'Provide valid support details using only the supported ticket fields.',
            );
        }
        ...
        try {
            ...
            if ($productReference !== null && $datasetFields === []) {
                return ToolResult::failure(
                    'invalid_arguments',
                    'Product context is not configured for this support integration.',
                );
            }
            ...
            if ($modelInputs === null) {
                return ToolResult::failure(
                    'invalid_arguments',
                    'The supplied support fields are not configured for this integration.',
                );
            }
            ...
            if ($operationArguments === null) {
                return ToolResult::failure(
                    'missing_product_data',
                    'The support request could not be enriched from the referenced product.',
                );
            }
            ...
            return $this->actionManager->propose(
                ...
                $operationArguments,
                $this->confirmationSummary($inputs, $resolvedProduct),
                $context,
            );
        } catch (Throwable $exception) {
            logger()->warning('AI support ticket proposal failed.', [
                'bot_id' => $bot->id,
                'team_id' => $bot->team_id,
    ...
    private function configuredMappings(BotApiOperation $attachment): ?array
    {
        ...
        foreach (array_keys($inputMapping) as $modelInput) {
            ...
            if ($modelInput !== 'product_reference' && $mapping['source'] !== 'model_input') {
                return null;
            }
    ...
    private function mappedInputs(array $inputs, array $mappings, bool $hasProduct): ?array
    {
        ...
        foreach ($inputs as $modelInput => $value) {
            ...
            if ($mapping === null || $mapping['source'] !== 'model_input') {
                return null;
            }
     ...
     */
    private function confirmationSummary(array $inputs, ?array $resolvedProduct): string
    {
        $subject = $inputs['subject'] ?? null;

        if ($subject !== null) {
            return 'Create a support ticket about “'.Str::limit($subject, 120, '').'”.';
        }

        $productLabel = $this->productLabel($resolvedProduct);

        if ($productLabel !== null) {
            return 'Create a support ticket about '.Str::limit($productLabel, 100, '').'.';
        }

        $category = $inputs['category'] ?? null;

        return $category === null
            ? 'Send this issue to the support team.'
            : 'Send this '.Str::limit($category, 100, '').' issue to the support team.';
    }
    ...
    private function integrationUnavailable(): ToolResult
    {
        return ToolResult::failure(
            ...
            'Support ticket creation is not currently available.',
        );
    }
}
