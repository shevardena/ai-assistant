...
type FieldRow = { name: string; path: string; required: boolean; type: FieldType; semantic_role: PriceSemanticRole | null };
...
const field = (): FieldRow => ({ name: '', path: '', required: true, type: 'string', semantic_role: null });
...
export default function GraphqlOperationCreate({ dataSource, templateContext }: Props) {
    ...
    const form = useForm({
        ...
        records_path: '', response_fields: [{ ...field(), name: 'result', required: true }] as FieldRow[],
...
return null;
    ...
    const updateField = (index: number, key: keyof FieldRow, value: string | boolean | null) => form.setData('response_fields', form.data.response_fields.map((row, rowIndex) => {
        if (rowIndex !== index) {
            return row;
        }

        const next = { ...row, [key]: value } as FieldRow;

        if (key === 'type' && !priceRoleOptions(value as FieldType).includes(row.semantic_role ?? 'current_price')) {
